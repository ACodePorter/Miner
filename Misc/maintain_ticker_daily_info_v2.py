...
class TickerDailyInfoMaintainer:
    ...
    def get_trading_dates_backwards(self, start_date: str | None = None, end_date: str | None = None, limit: int | None = None) -> List[str]:
        ...
        Args:
            ...
            end_date: End date in YYYYMMDD format, defaults to latest available
        ...
        try:
            ...
            trade_dates = TickerDailyInfo.objects(trade_date__gte=datetime.datetime.strptime(start_date, '%Y%m%d'), trade_date__lte=datetime.datetime.strptime(end_date, '%Y%m%d')).distinct('trade_date')
            _logger.info(f"Found {len(trade_dates)} unique trade dates")
            _logger.info(f"First few trade dates: {trade_dates[:5] if trade_dates else 'None'}")
            _logger.info(f"Type of first trade date: {type(trade_dates[0]) if trade_dates else 'None'}")
            ...
            # Convert datetime objects to YYYYMMDD format
            result = []
            for dt in trade_dates:
                try:
                    if hasattr(dt, 'strftime'):
                        ...
                        # _logger.warning(f"Unexpected type for trade_date: {type(dt)}, value: {dt}")
                        # Try to convert if it's a string or other format
                        if isinstance(dt, str):
                            # If it's already in YYYYMMDD format, use it directly
                            if len(dt) == 8 and dt.isdigit():
                                result.append(dt)
                            else:
                                # Try to parse it
                                parsed_dt = datetime.datetime.strptime(dt, '%Y,%m,%d,%H,%M,%S,%f')
                                result.append(parsed_dt.strftime('%Y%m%d'))
                        else:
                            _logger.error(f"Cannot convert trade_date: {dt}")
                except Exception as e:
                    _logger.error(f"Error converting trade_date {dt}: {e}")
    ...
    def find_duplicates_for_date(self, trade_date: str) -> Dict[str, List[TickerDailyInfo]]:
        ...
        try:
            ...
            duplicates = {key: docs for key, docs in grouped.items() if len(docs) > 1}
            
            return duplicates
            
        except Exception as e:
            _logger.error(f'Failed to find duplicates for date {trade_date}: {e}')
            return {}
    ...
    def remove_duplicates(self, duplicates: Dict[str, List[TickerDailyInfo]], dry_run: bool = True) -> Dict[str, int]:
        ...
        for (ticker, interval), docs in duplicates.items():
            ...
            docs.sort(key=lambda x: x.id.generation_time if hasattr(x.id, 'generation_time') else x.id)
            
            # Keep the first document, remove the rest
            docs_to_remove = docs[1:]
            
            if dry_run:
                _logger.info(f'[DRY RUN] Would remove {len(docs_to_remove)} duplicates for {ticker} ({interval}) on {docs[0].trade_date.strftime("%Y-%m-%d")}')
                for doc in docs_to_remove:
                    _logger.info(f'  [DRY RUN] Would remove: {doc.id} - {doc.ticker} {doc.interval} {doc.trade_date}')
            else:
                for doc in docs_to_remove:
                    try:
                        doc.delete()
                    except Exception as e:
                        _logger.error(f'Failed to remove document {doc.id}: {e}')
                        continue
    ...
    def maintain_collection(self, start_date: str = "19620102", end_date: str = "20250712", limit: int | None = None, dry_run: bool = True) -> Dict[str, Any]:
        ...
        """
        _logger.info(f'Starting TickerDailyInfo maintenance (dry_run={dry_run})')
        if start_date:
        ...
        # Get trading dates
        trading_dates = self.get_trading_dates_backwards(start_date, end_date, limit)
        ...
        _logger.info(f'Trading dates processed: {overall_stats["trading_dates_processed"]}')
        _logger.info(f'Dates with duplicates: {overall_stats["dates_with_duplicates"]}')
        _logger.info(f'Total groups with duplicates: {overall_stats["total_groups_with_duplicates"]}')
        _logger.info(f'Total documents removed: {overall_stats["total_documents_removed"]}')
        _logger.info('=' * 50)
...
def distribute_dates(trading_dates: List[str], num_processes: int) -> List[tuple]:
    ...
    for i in range(0, len(trading_dates), chunk_size):
        ...
        if chunk:
            ...
            end_date = chunk[0]
            limit = len(chunk)
            chunks.append((start_date, end_date, limit, True))  # dry_run=True for safety
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Maintain TickerDailyInfo collection by removing duplicates')
    parser.add_argument('--start-date', type=str, help='Start date in YYYYMMDD format (default: latest available)')
    parser.add_argument('--end-date', type=str, help='End date in YYYYMMDD format (default: latest available)')
    parser.add_argument('--limit', type=int, help='Maximum number of trading dates to process (default: all)')
    parser.add_argument('--dry-run', action='store_true', default=True, help='Dry run mode (default: True)')
    parser.add_argument('--execute', action='store_true', help='Actually execute the changes (overrides --dry-run)')
    ...
    if dry_run:
        ...
        _logger.warning('Running in EXECUTE mode - changes will be made to the database')
    ...
    date_chunks = [(start_date, end_date, limit, dry_run) for start_date, end_date, limit, _ in date_chunks]
