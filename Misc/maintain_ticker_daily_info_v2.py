...
class TickerDailyInfoMaintainer:
    ...
    def get_trading_dates_backwards(self, start_date: str | None = None, end_date: str | None = None, limit: int | None = None) -> List[str]:
        ...
        try:
            ...
            trade_dates = TickerDailyInfo.objects(trade_date__gte=datetime.datetime.strptime(
                start_date, '%Y%m%d'), trade_date__lte=datetime.datetime.strptime(end_date, '%Y%m%d')).distinct('trade_date')
            _logger.info(f"Found {len(trade_dates)} unique trade dates")
            _logger.info(
                f"First few trade dates: {trade_dates[:5] if trade_dates else 'None'}")
            _logger.info(
                f"Type of first trade date: {type(trade_dates[0]) if trade_dates else 'None'}")
